...
> 写作标准：**写给失忆的读者**——断上下文的新会话/新同事只凭这段就能把环境跑起来，
> 不求助、不迷路。把脆弱点里 env/dependency 类知识聚合成人读版。
...
> planned_changes 是集合，这里是序列。每步结尾挂一个**可观察的里程碑**——
> 做完一步就看到一步的响，不允许"全部写完再一起调试"。
...
> verify 失败 ≠ 停下硬重试：**先查这张表**。左列贴真实报错的关键词，右列给自救命令
> （与 fragile_points 的 `rescue:` 字段同源；踩坑修复后回填，经 /regress:learn
> 沉淀为 AGENTS.md 项目规律——预先写的自救与事后学出的规律在此合流）。
...
> 病例（REGRESS-005 对讲音频）：三轮"根治"全上，真凶却是自己上一轮写的缓冲参数；
> 最大一次误诊已写出整套规格推责前端，被用户两个字扭转——"看日志"。
> 瓶颈从来不是权限，是顺序：**下结论之前先测量。**
...
假设被实测证伪时（如协议推测被真机推翻），**不要悄悄改写 assumptions 就完事**——
追加一行失效记录，把"原来以为什么、实际是什么、证据是什么"留在产物里：
