...
- **批内全治理环不豁免**：清单→顾问预审（队列级 consult 一次覆盖+批级引用化石，
  新决策点才追加咨询）→临行→实现→门禁→发布→done 推送→落账（**每条裁决附
  解释边界一行：「本裁决不许被引用来证明 X」——防裁决被越界引用**，v1.88.3/102
  better-harness 借抄；纪律位，实践两批稳后议升必填）。
- **批前自驳自查两问**（v1.88.3/102——diff 可见后、提交前跑；预审期只能填猜测
  故不进 5a）：①计划里哪些约束是模型可读软约束，各自的硬通道（exit 2/机器位）
  在哪？没有硬通道的记观察位；②哪个假设被证伪整个计划作废？有没有更早发现
  的量测位？跳过两次即升 gate 硬通道（升格触发器与批 100 同款）。
...
## 宣称纪律层（v1.88.1：宣称的强度被证据的强度封顶——better-harness 深研借抄）

治"叙事先行"：报告里的效果动词不许跑到证据前面。速览（唯一属主：
[claim-discipline.md](claim-discipline.md)）：

1. **计数≠行为**：遥测计数只 bound 范围不证明效果；引用计数做有效性宣称必须
   降格为线索并点名待补证据。
2. **负面宣称清单**：每份批报告/巡检报告末尾必含「本证据不支持的宣称」一节，
   **至少一条**（空节=纸面化，连续两批空节升机械位——预审裁决）。
3. **宣称-证据对齐**：效果动词须能指回已证活以上证据；证据七态词汇
   （已配置/已接线/已证活/已证效/缺/未观测/不适用）见 claim-discipline.md 映射表。

判断合并两条（多源候选/巡检发现/子代理产出）：**保留优先**——四要素
（目标+后果+属主+修复路径）全同才合并，丢弃留由，绝不为整洁砍真候选；
**冻结先于叙事**——判断冻结后才写叙事，叙事不许反哺判断。
...
### 深查节规约（v1.93.0/121，M/L 档）

清单验收标准之后加「## 深查节」：三反问各一行实答（验证位错位/判据外推/心虚
探测）。门禁 6.5b 在 M/L done 时查**结构在场**（三问+非占位答）——质量留纪律，
照抄问题原文不算答。这是把用户的「深入审查，是否还有遗漏」提前替用户说：
对账查询之外强制跑一次覆盖查询。
